...
- **FIXED (2026-09-01)**: `expiry` is now circuit public input `[39]` (upstream fix, zexoverz/confidential-agent-policy-verdicts commit `d950ac1422cf79bafff11fcfb62c3e8b4ce3d782`, tracked at zexoverz/confidential-agent-policy-verdicts#3). A `verify` result of `true` now establishes that the supplied `expiry` is the one the circuit actually authorized, not just that it has not lapsed on-chain -- both the freshness check and the circuit binding hold.
